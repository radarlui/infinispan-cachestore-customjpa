...
public class JpaStore implements AdvancedLoadWriteStore {
   ...
   public void start() {
	   if (emfRegistry == null)
	       emfRegistry = new EntityManagerFactoryRegistry();
      try {
    	  Thread.currentThread().setContextClassLoader(JpaStore.class.getClassLoader());
    	 this.emf = Persistence.createEntityManagerFactory(configuration.persistenceUnitName());
      ...
      try {
         ...
         throw new JpaStoreException("Entity class [" + this.configuration.entityClass() + " specified in configuration is not recognized by the EntityManagerFactory with Persistence Unit [" + this.configuration.persistenceUnitName() + "]", e);
      }
